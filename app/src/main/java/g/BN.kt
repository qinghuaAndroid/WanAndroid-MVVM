...
@HiltViewModel
class BN @Inject constructor(private val mRepository: BM) :
    BaseViewModel() {

    private val _uiState = MutableLiveData<BaseUiState<MutableList<CA>>>()
    val uiState: LiveData<BaseUiState<MutableList<CA>>>
        get() = _uiState

    private val _resultsChange = MutableLiveData<ResultsChange<CE>>()
    val resultsChange: LiveData<ResultsChange<CE>>
        get() = _resultsChange

    fun queryAll() {
        viewModelScope.launch {
            mRepository.queryAll()
                .collect { resultsChange: ResultsChange<CE> ->
    ...
    private fun emitUiState(
        ...
        showSuccess: MutableList<CA>? = null
    ) {
